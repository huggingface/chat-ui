...
describe("marked html video tag support", () => {
	...
	test("escapes non-video html tags", () => {
	...
	test("allows <audio> with multiple source tags and fallback text", () => {
		const input = `<audio controls>
...
Your browser does not support the audio tag
		...
		expect(html).toContain("<audio");
		expect(html).toContain('src="horse.ogg"');
		expect(html).toContain('src="horse.mp3"');
