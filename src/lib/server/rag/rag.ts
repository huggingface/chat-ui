...
type RetrieveRagContextFn<T extends RagContext = RagContext> = (conv: Conversation, prompt: string, updatePad: (upd: MessageUpdate) => void) => Promise<T>;

type BuildPromptFn<T extends RagContext = RagContext> = (messages: BuildPromptMessage[], context: T) => BuildPromptMessage[];
