...
export function buildToolPreprompt(tools: OpenAiTool[]): string {
	...
	return [
		...
		`For search queries: use few precise keywords (3-6 words), avoid redundant synonyms, use the exact terminology the source would use (e.g., "heroes" not "characters" for Overwatch), prefer relative time terms like "latest" or "newest" over specific years.`,
