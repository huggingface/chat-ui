...
export function buildToolPreprompt(tools: OpenAiTool[]): string {
	...
	if (names.includes("display_gallery")) {
		lines.push(
			`GALLERY: Use display_gallery to present multiple media items (images, videos, audio) in a visual gallery. Provide items with url, media_type, and optionally title/description. For a single image, prefer inline markdown ![alt](url) instead.`
		);
	}
	return lines.join(" ");
}
