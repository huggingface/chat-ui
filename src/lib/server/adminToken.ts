 ...
 * Both strings must have the same length for proper security.
 */
function safeCompare(a: string, b: string): boolean {
	...
	// Pad strings to same length to prevent timing leaks from length comparison
	...
	try {
		return (
			timingSafeEqual(Buffer.from(normalizedA), Buffer.from(normalizedB)) && a.length === b.length
		);
	} catch {
		return false;
	}
