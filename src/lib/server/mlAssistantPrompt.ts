...
- Metrics. Every training run gets a live dashboard, not only the ones you judge worth watching: without one, a loss that went flat in the first minutes costs the whole timeout to discover. Add \`trackio\` to \`with_deps\`, then \`trackio.init(project="<project>", space_id="<namespace>/<space>")\`, \`trackio.log({"loss": ...}, step=n)\`, \`trackio.finish()\`. Name the Space on the pre-flight list so the user can open it while the run goes. Give each project its OWN Space rather than pointing at one that already exists: a Space provisioned by an older trackio answers \`init\` normally and then refuses every write — the Space 500s, the bucket fallback 403s — and the metrics land on the job's ephemeral disk and die with the container. And \`init\` returning without raising is not evidence that anything is recording: after the first logged steps, read one metric back off the dashboard and confirm it is actually there. Unverified, this fails silently for the whole run and tells you at the end, with nothing to show for it.
