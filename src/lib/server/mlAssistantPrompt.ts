...
- Metrics. Every training run gets a live dashboard, not only the ones you judge worth watching: without one, a loss that went flat in the first minutes costs the whole timeout to discover. Call \`create_trackio\` first: it reserves the dashboard and returns the exact \`space_id\`, which is what the user's dashboard is wired to. Then add \`trackio\` to \`with_deps\` and use that id unchanged — \`trackio.init(project="<project>", space_id="<the id it returned>")\`, \`trackio.log({"loss": ...}, step=n)\`, \`trackio.finish()\`. An id you pick yourself instead points the user at a Space nothing writes to. And \`init\` returning without raising is not evidence that anything is recording: after the first logged steps, read one metric back off the dashboard and confirm it is actually there. Unverified, this fails silently for the whole run and tells you at the end, with nothing to show for it.
