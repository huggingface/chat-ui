...
<div
	class="group flex flex-none rounded-lg pl-2 pr-1.5 text-base text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 sm:text-sm
		{showDescription ? 'flex-col py-1.5' : 'h-8 items-center gap-1.5 max-sm:h-10'}
		{conv.id === page.params.id ? 'bg-gray-100 dark:bg-gray-700' : ''}"
>
	<div
		class="flex w-full {showDescription ? 'items-center gap-1.5' : 'h-full items-center gap-1.5'}"
	>
		{#if inlineEditing}
			<input
				bind:this={inputEl}
		...
		{:else}
			<a
				...
				class="min-w-0 flex-1 truncate {showDescription ? 'py-0.5' : 'py-2'} first-letter:uppercase"
				onclick={(e) => {
					if (e.detail >= 2) {
						e.preventDefault();
						startInlineEdit();
	...
	{#if showDescription && description}
		<p class="mt-0.5 line-clamp-1 pl-0 text-xs text-gray-500 dark:text-gray-400">
			<!-- eslint-disable-next-line svelte/no-at-html-tags -- input is HTML-escaped in highlight() -->
			{@html highlight(description, searchInput)}
		</p>
	{/if}
</div>
