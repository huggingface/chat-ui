					...
					>
						...
						{#if loading}
							<StopGeneratingBtn
								onClick={() => onstop?.()}
								showBorder={true}
								classNames="absolute bottom-2.5 right-2.5 size-10 sm:size-9 self-end rounded-full border bg-white text-black shadow transition-none dark:border-transparent dark:bg-gray-600 dark:text-white"
							/>
						{:else}
							{#if transcriptionEnabled}
								<button
									type="button"
									class="btn absolute bottom-2.5 right-[54px] size-10 self-end rounded-full border bg-white/50 text-gray-500 transition-none hover:bg-gray-50 hover:text-gray-700 dark:border-transparent dark:bg-gray-600/50 dark:text-gray-300 dark:hover:bg-gray-500 dark:hover:text-white sm:right-12 sm:size-9"
									disabled={isReadOnly}
									onclick={() => {
										isRecording = true;
									}}
								...
								>
									<IconMic class="size-5 sm:size-4" />
								</button>
							{/if}
							<button
								class="btn absolute bottom-2.5 right-2.5 size-10 self-end rounded-full border bg-white text-black shadow transition-none enabled:hover:bg-white enabled:hover:shadow-inner dark:border-transparent dark:bg-gray-600 dark:text-white dark:hover:enabled:bg-black sm:size-9 {!draft ||
								isReadOnly
									? ''
								...
								name="submit"
							>
								<IconArrowUp class="size-5 sm:size-4" />
							</button>
						{/if}
					</div>
				{/if}
