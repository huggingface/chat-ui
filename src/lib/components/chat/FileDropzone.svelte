			...
			if (event.dataTransfer.items[0].kind === "file") {
				...
				if (file) {
					if (!event.dataTransfer.items[0].type.startsWith("image")) {
						file_error = true;
						file_error_message = "Only images are supported";
						files = [];
						return;
					}
...
>
	<div class="object-center">
		...
		{/if}
		<div class="mt-3 flex justify-center" class:opacity-0={file_error}>
			<CarbonImage class="text-5xl text-gray-500 dark:text-gray-400" />
		</div>
		<p class="mb-3 mt-3 text-sm text-gray-500 dark:text-gray-400" class:opacity-0={file_error}>
